...
def main():
    ...
    for ear_type in ear_types:
        ...
        for name in names:
            ...
            # for deg in degrees:
            imgs = os.listdir(os.path.join(keypoint_, ear_type, "7_result", name, "5_test_img"))
            ...
            df = pd.DataFrame()
            for img in imgs:
                pred_instances = process_one_image(os.path.join(keypoint_, ear_type, "7_result", name, "5_test_img", img), detector, pose_estimator)
            ...
            for deg in degrees:
                ...
                if not os.path.isdir(os.path.join(keypoint_, ear_type, "7_result", name, "pred", deg)):
                    os.makedirs(os.path.join(keypoint_, ear_type, "7_result", name, "pred", deg))
                df.iloc[count:count+frame_count,:].to_csv(os.path.join(keypoint_, ear_type, "7_result", name, "pred", deg, "pred.csv"))
